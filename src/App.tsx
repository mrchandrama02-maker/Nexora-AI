...
const AuthModal = ({ isOpen, onClose, isDarkMode, showToast }: { isOpen: boolean, onClose: () => void, isDarkMode: boolean, showToast: (msg: string, type?: any) => void }) => {
...
const AdminDashboard = ({ isDarkMode, onClose }: { isDarkMode: boolean, onClose: () => void }) => {
  ...
  const toggleFavorite = async (toolId: string) => {
    ...
    try {
      if (isFavorite) {
        ...
        setUserData(prev => ({ ...prev, favorites: prev.favorites.filter(id => id !== toolId) }));
        showToast('Removed from favorites', 'info');
      } else {
        await updateDoc(userRef, {
          favorites: arrayUnion(toolId)
        });
        setUserData(prev => ({ ...prev, favorites: [...prev.favorites, toolId] }));
              ...
              {userData.user ? (
                <div className="flex items-center gap-3 pl-2 border-l border-zinc-200 dark:border-zinc-800">
                  <div className="text-right hidden md:block">
                    <p className="text-xs font-black tracking-tight leading-none mb-1">{userData.user.name}</p>
                  ...
                  <div className="w-10 h-10 rounded-2xl overflow-hidden border-2 border-indigo-600/20">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-sm font-black transition-all shadow-lg shadow-indigo-600/20"
                >
                  <UserIcon size={18} />
                  Login
                </button>
